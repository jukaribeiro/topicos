...
uint8_t crescente = 1; //define a ordem das luzes
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
      ...
      if (crescente==1){
    	  if (adc_value>=820){
    		  HAL_GPIO_WritePin(BRANCO_GPIO_Port, BRANCO_Pin, GPIO_PIN_SET);
    	  }
    	  else{
    		  HAL_GPIO_WritePin(BRANCO_GPIO_Port, BRANCO_Pin, GPIO_PIN_RESET);
    	  }
    	  if (adc_value>=1639){
    		  HAL_GPIO_WritePin(VERMELHO_GPIO_Port, VERMELHO_Pin, GPIO_PIN_SET);
    	  }
    	  else{
    		  HAL_GPIO_WritePin(VERMELHO_GPIO_Port, VERMELHO_Pin, GPIO_PIN_RESET);
    	  }
    	  if (adc_value>=2458){
    		  HAL_GPIO_WritePin(AMARELO_GPIO_Port, AMARELO_Pin, GPIO_PIN_SET);
    	  }
    	  else{
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, BRANCO_Pin|VERMELHO_Pin|AMARELO_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(VERDE_GPIO_Port, VERDE_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : BRANCO_Pin VERMELHO_Pin AMARELO_Pin */
  GPIO_InitStruct.Pin = BRANCO_Pin|VERMELHO_Pin|AMARELO_Pin;
